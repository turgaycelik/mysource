...
activeObjsPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'ActiveObjects Plugin', key: 'AOP', description: '') {
        ...
        runBundledPluginTestsScript(
                argument: '-g com.atlassian.jira.plugins -a jira-bamboo-plugin -j ${bamboo.plugin.builds.jira.trunk.version} -X -i --force-mvn-version=3 -- ${bamboo.sandbox.cli.options}',
                environmentVariables: 'MAVEN_OPTS="-Xmx512m -XX:MaxPermSize=256m" M3_HOME="${bamboo.capability.system.builder.mvn3.Maven 3.2}" JAVA_HOME="${bamboo.capability.system.jdk.JDK 1.6}"'
        )
...
jiraBambooPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'JIRA Bamboo Plugin', key: 'JBP', description: '', enabled: "false") {
        requireBash()
        requireJdk(jdk: '#jdk')
        requireMaven2()
        requirePython()
...
jiraDvcsPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'JIRA DVCS plugin', key: 'JDVCSP', description: 'Bundled plugin tests for DVCS plugin') {
        ...
        runBundledPluginTestsWithErrorCheckScript(
                argument: '-g com.atlassian.jira.plugins -a jira-dvcs-connector-plugin -j ${bamboo.plugin.builds.jira.stable.version} -X -- ${bamboo.sandbox.cli.options} -Djirabitbucketconnector.password=dvcsconnector23 -Ddvcsconnectortest.password=dvcsconnector23',
                environmentVariables: 'MAVEN_OPTS="-Xmx512m -XX:MaxPermSize=256m" M2_HOME="${bamboo.capability.system.builder.mvn2.Maven 2.1}" JAVA_HOME="${bamboo.capability.system.jdk.#jdk}"'
        )
        parseBundledPluginJUnitResults()
    }
...
jiraHtml5AttachImagesPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'JIRA HTML5 Attach Images', key: 'JHAIP', description: '') {
        requireBash()
        requireJdk(jdk: '#jdk')
        requireMaven2()
        requireMaven30()
...
jiraICalendarPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'JIRA iCalendar Plugin', key: 'JICP', description: '') {
        ...
        checkoutInDir(repoName: '#accTestRunnerRepoName', dir: 'runner')
        runBundledPluginTestsScript(
                argument: '-g com.atlassian.jira.extra -a jira-ical-feed -j ${bamboo.plugin.builds.jira.stable.version}  -- ${bamboo.sandbox.cli.options}',
                environmentVariables: 'MAVEN_OPTS="-Xmx512m -XX:MaxPermSize=256m" M2_HOME="${bamboo.capability.system.builder.mvn2.Maven 2.1}" JAVA_HOME="${bamboo.capability.system.jdk.#jdk}"'
        )
        parseBundledPluginJUnitResults()
    }
...
jiraIssueCollectorPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'JIRA Issue Collector Plugin', key: 'JICOLP', description: '') {
        ...
        checkoutInDir(repoName: '#accTestRunnerRepoName', dir: 'runner')
        runBundledPluginTestsScript(
                argument: '--start-x-display --x-display-geometry 1600x1200 -g com.atlassian.jira.collector.plugin -a jira-issue-collector-plugin -j ${bamboo.plugin.builds.jira.stable.version} -i -ta jira-issue-collector-plugin-it  -- ${bamboo.sandbox.cli.options}',
                environmentVariables: 'MAVEN_OPTS="-Xmx512m -XX:MaxPermSize=256m" M2_HOME="${bamboo.capability.system.builder.mvn2.Maven 2.1}" JAVA_HOME="${bamboo.capability.system.jdk.#jdk}"'
        )
        parseBundledPluginJUnitResults()
    }
...
myWorkPlugin(['accTestRunnerRepoName', 'jdk']) {
    job(name: 'MyWork Plugins - Disabled', enabled: "false", key: 'MWP', description: 'Tests for MyWork JIRA integration - NOT JDK8 compliant and https://jdog.jira-dev.com/browse/JDEV-29025') {
        requireBash()
        requireJdk(jdk: '#jdk')
        requireMaven2()
        requireMaven30()
