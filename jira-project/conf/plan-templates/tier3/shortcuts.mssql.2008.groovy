...
mssqlBatch(['id', 'batch', 'repoName', 'jdk']) {
    job(name: 'MSSQL2008 Batch #id', key: 'MSSQL#id', description: '') {
...
runMssqlBatch(['jdk', 'batch']) {
    task(
            ...
            goal: '-f jira-distribution/tpm-standalone/spm/pom.xml clean install -DJIRA=Trunk -DStandalone -DIntegrationTest=JIRA -DnumberOfBatches=${bamboo.batch.max} -Dbatch=#batch -DMSSQL -Dsnapshot=${bamboo.jira.6.3.branch.version} -B -e',
