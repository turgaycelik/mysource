...
globalVariables() {
    ...
    variable(key: 'run.webdriver.test.1', value: 'verify -B -PhallelujahClient,use-artifact-passing,maven3-statistics -Djira.security.disabled=true -Djira.scheduler.RamJobStore=true -Dsystem.bamboo.agent.home=${bamboo.system.bamboo.agent.home} -Dmaven.test.unit.skip=true -Djava.awt.headless=true')
...
miscellaneousConfig() {
    ...
    // TODO: Misc config -> There is no support on plan templates to enable the bisect plugin
}

produceWebDriverArtifacts(['repoName', 'jdk']) {
    job(name: 'Produce JIRA artifacts', key: 'ARTIFACTS', description: '') {
        requireLinux()
        requireMaven30()
        requireJdk(jdk: '#jdk')
...
webDriverTestsJob(['index', 'jdk']) {
    job(name: 'WebDriver Batch #index', key: 'WEBDRIVER#index', description: '') {
        requireLinux()
        requireMaven2()
        requireMaven30()
        requireJdk(jdk: '#jdk')
...
webDriverTestsHallelujahServer(['jdk']) {
    job(name: 'WebDriver Hallelujah Server', key: 'WDSERVER', description: '') {
        requireLinux()
        requireMaven2()
        requireMaven30()
        requireJdk(jdk: '#jdk')
