planDependencies() {
    dependencies(triggerOnlyAfterAllStagesGreen: 'true', automaticDependency: 'false', triggerForBranches: 'false') {
        childPlan(planKey: 'J63STABLEPT-DEPLOYJIRA')
    }
}
...
produceFuncArtifacts(['repoName', 'jdk']) {
    job(name: 'Produce artifacts', key: 'ARTIFACTS', description: '') {
        requireLinux()
        requireMaven30()
        requireJdk(jdk: '#jdk')
...
funcTestsJob(['index', 'jdk']) {
    job(name: 'Func #index', key: 'FUNC#index', description: 'Func #index') {
        requireLinux()
        requireMaven30()
        requireJdk(jdk: '#jdk')
...
funcTestsHallelujahServer(['jdk']) {
    job(name: 'Hallelujah Test Server', key: 'HALLELUJAHTESTSERVER', description: 'Hallelujah test server') {
        requireLinux()
        requireMaven30()
        requireJdk(jdk: '#jdk')
...
qUnitTests(['repoName', 'jdk']) {
    job(name: 'QUnit Tests', key: 'QUNIT', description: 'Runs QUnit tests') {
        requireLocalLinux()
        requireMaven2()
        requireMaven30()
        requireJdk(jdk: '#jdk')
...
runQUnitTests(['jdk']) {
    task(
            ...
            buildJdk: '#jdk',
            mavenExecutable: 'Maven 3.0',
            environmentVariables: 'DISPLAY=":20" MAVEN_OPTS="-Xmx512m -XX:MaxPermSize=256m" M2_HOME="${bamboo.capability.system.builder.mvn3.Maven 3.0}"',
            hasTests: 'true',
...
unitTestsAndFunctionalUnitTests(['repoName', 'jdk']) {
    job(name: 'Unit Tests plus Functional Unit Tests', key: 'FUNCUNIT', description: 'Our regular Unit Test build plus the Unit Tests that test the Functional Tests') {
        requireLinux()
        requireMaven30()
        requireJdk(jdk: '#jdk')
