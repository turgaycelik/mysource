...
SNAPSHOT_NAME="installer-test-baseline"
TEST_DIR="jira-installer-test"
INSTALLER_CONFIG="JIRA_windows.varfile"
TEST_SCRIPT="testInstaller.sh"
BASE=$(pwd)
...
function prep_vm() {
    ...
    if [[ ! -e ../func_tests ]]; then
        echo "expected to find func tests in sister directory"
        exit 1
    fi
    tar --exclude .svn -jcf target/func_tests.tbz2 ../func_tests
...
function run_guest_script() {
    ...
    vmrun -gu $GUEST_USERNAME -gp $GUEST_PASSWORD "runProgramInGuest"  "$vm" \
        ...
        "/c c:\\cygwin\\bin\\bash.exe --login c:\\$TEST_DIR\\testInstaller.sh $installer /cygdrive/c/$TEST_DIR/results.txt $edition"
}
