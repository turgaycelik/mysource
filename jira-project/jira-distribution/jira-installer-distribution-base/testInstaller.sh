...
pushd func_tests
localtest_properties="src/etc/java/localtest.properties.standalone.$edition.installer"
if [[ ! -e $localtest_properties ]]; then 
	echo "can't find $localtest_properties (edition is $edition, are you sure you wanted this?)"
fi
echo "maven clean" >>$outfile
maven clean >>$outfile
echo "maven test" >>$outfile
maven test -Dmaven.junit.fork=once -Dtest.server.properties=C:/jira-installer-test/func_tests/$localtest_properties >>$outfile 2>&1
if (( $? == 0 )); then
...
pushd "func_tests/target"
    tar rvf "${output_tar}" "test-reports" >>$outfile 2>&1 || echo "Could not tar the test results." >>$outfile
popd

# Add the logs to the tar file.
pushd "${JIRA_HOME}"
    tar rvf "${output_tar}" "log" >>$outfile 2>&1 || echo "Could not tar the JIRA logs." >>$outfile
    tar rvf "${output_tar}" "import" >>$outfile 2>&1 || echo "Could not tar the JIRA import directory." >>$outfile
popd
