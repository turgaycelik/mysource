...
NAME="${product.name} Configuration Tool"
NAME_SWITCH="-Djira.configurator.name=$NAME"

# Run the Configurator Java class
if $macosx ; then
    "$_RUNJAVA" -Xdock:name="$NAME" -Dapple.laf.useScreenMenuBar=true "$NAME_SWITCH" -classpath "$CP" "$MAIN" "$@"
else
    "$_RUNJAVA" "$NAME_SWITCH" -classpath "$CP" "$MAIN" "$@"
fi
