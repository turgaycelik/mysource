...
public class CascadingSelectCustomFieldOperationsHandler extends AbstractCustomFieldOperationsHandler<Map<String, Option>>
{
    ...
    protected Map<String, Option> handleSetOperation(IssueContext issueCtx, Issue issue, Map<String, Option> currentFieldValue, JsonData operationValue, ErrorCollection errors)
    {
        ...
        if (child != null)
        {
            options.put(CascadingSelectCFType.CHILD_KEY, child);
        }
        return options;
    }
    ...
    protected void finaliseOperation(Map<String, Option> finalValue, IssueInputParameters parameters, ErrorCollection errors)
    {
        if (finalValue != null)
        {
            ...
            {
                parameters.addCustomFieldValue(field.getId(), finalValue.get(CascadingSelectCFType.PARENT_KEY).getOptionId().toString());
            }
            if (finalValue.get(CascadingSelectCFType.CHILD_KEY) != null)
            {
                parameters.addCustomFieldValue(field.getId() + ":1", finalValue.get(CascadingSelectCFType.CHILD_KEY).getOptionId().toString());
            }
        }
        else
        {
