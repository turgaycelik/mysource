...
public class UpgradeTask_Build851 extends AbstractUpgradeTask
{
    ...
    public UpgradeTask_Build851(OfBizDelegator ofBizDelegator, JiraAuthenticationContext authContext, UserManager userManager)
    {
        super(false);
    ...
    private void migrateFeatures(String featureType) throws GenericEntityException
    {
        ...
        if (sourcePropertyGV != null)
        {
            ...
            if (sourceFeatureGV != null)
            {
                ...
                {
                    MapBuilder<String, Object> propertyMapBuilder = MapBuilder.<String, Object>newBuilder();
    ...
    private String getUserKey(String featureType, long entityId)
    {
        ...
        {
            GenericValue usergv = ofBizDelegator.findById(Table.EXTERNAL_ENTITY,entityId);
            if (usergv != null)
            {
                String username = usergv.getString(Column.USERNAME);
                return ApplicationUsers.getKeyFor(userManager.getUserEvenWhenUnknown(username));
            }
        }
